document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function getErrorMessage(field) {
      ...
      if ((type === 'tel' || name.includes('phone') || name.includes('tel')) && value !== '') {
      ...
      if (field.maxLength && field.maxLength > 0 && value.length > field.maxLength) {
        return (field.dataset.errorMaxlength || 'Maximum {max} characters allowed.').replace('{max}', field.maxLength);
      }
    ...
    function showError(field, message) {
      ...
      field.setAttribute('aria-describedby', errorEl.id || (errorEl.id = 'error-' + Math.random().toString(36).slice(2)));
    }
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      const action = contactForm.action || window.location.href;
      ...
      fetch(action, {
        ...
        .then(function (response) {
          if (response.ok) {
            const successMsg = contactForm.dataset.successMessage || 'Thank you! Your message has been sent.';
            showFormMessage(contactForm, successMsg, 'success');
            contactForm.reset();
            contactForm.querySelectorAll('.' + FIELD_SUCCESS_CLASS).forEach(function (el) {
              el.classList.remove(FIELD_SUCCESS_CLASS);
            });
          } else {
        ...
        .catch(function () {
          const errorMsg = contactForm.dataset.errorMessage || 'Something went wrong. Please try again.';
    ...
    function showFormMessage(form, message, type) {
      ...
      msgEl.className = 'form-status-message form-status-message--' + type;
  ...
  if (staggerContainers.length > 0 && 'IntersectionObserver' in window) {
    ...
    staggerContainers.forEach(function (container) {
      Array.from(container.children).forEach(function (child) {
        child.classList.add(ANIMATE_CLASS);
      });
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '602c9df4-a053-4ed5-8ce0-bbb9eaff85e2',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '602c9df4-a053-4ed5-8ce0-bbb9eaff85e2';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '602c9df4-a053-4ed5-8ce0-bbb9eaff85e2';
